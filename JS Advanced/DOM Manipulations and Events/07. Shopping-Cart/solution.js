function solve() {
  ...
  Array.from(addButtons).map((b) =>
    b.addEventListener("click", (b) => shoppingCard(b))
  );
  ...
  function shoppingCard(button) {
    let productName =
      button.path[2].getElementsByClassName("product-title")[0].innerText;

    let productPrice =
      +button.path[2].getElementsByClassName("product-line-price")[0].innerText;

    textArea.innerHTML += `Added ${productName} for ${productPrice.toFixed(
      2
    )} to the cart.\n`;
  ...
  function printFinalSumAndDisableButtons() {
    ...
    textArea.innerHTML += `You bought ${productNames.join(
      ", "
